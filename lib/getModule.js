...
module.exports = async function({ fullPath, name }, { env, log }) {
  ...
  const example = function(wrapStart = '', wrapEnd = '', makeSafe = true) {
    let out = '';
    if (Array.isArray(exampleData)) {
      out = exampleData.map(d => `${wrapStart}${renderRaw(d)}${wrapEnd}`).join('');
    } else {
      out = `${wrapStart}${renderRaw(exampleData)}${wrapEnd}`;
    }
