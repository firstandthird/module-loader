...
module.exports = async function({ fullPath, name }, { env, log }) {
  ...
  const example = function(wrapStart = '', wrapEnd = '') {
    if (Array.isArray(exampleData)) {
      return safe(exampleData.map(d => `${wrapStart}${renderRaw(d)}${wrapEnd}`).join(''));
    }
    return safe(`${wrapStart}${renderRaw(exampleData)}${wrapEnd}`);
  };
