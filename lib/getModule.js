...
module.exports = async function({ fullPath, name }, { env, log, debug }) {
  ...
  const example = function(makeSafe = true, data = exampleData) {
    let out = '';
    if (Array.isArray(data)) {
      out = data.map(d => example(makeSafe, d)).join('');
    } else {
      const config = data._config;
      delete data._config;
      const wrapStart = config ? config.wrapStart : '';
      const wrapEnd = config ? config.wrapEnd : '';
      const heading = config && config.name ? `<h2>${config.name}</h2>` : '';
      out = `${heading}${wrapStart}${renderRaw(data)}${wrapEnd}`;
    }
    return makeSafe ? safe(out) : out;
  };
